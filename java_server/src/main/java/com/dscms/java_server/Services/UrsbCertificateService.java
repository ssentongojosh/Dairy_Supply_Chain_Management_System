...
public class UrsbCertificateService {
    ...
    public boolean isVerified(MultipartFile ursbCertificate) {
      System.out.println("Verifying URSB Certificate...\n");
        ...
        // Load the logo templates
        Mat coatOfArmsLogo = Imgcodecs.imread("./src/main/resources/Images/COA.jpeg", Imgcodecs.IMREAD_GRAYSCALE);
        Mat ursbLogo = Imgcodecs.imread("./src/main/resources/Images/URSB-LOGO.jpg");
        ...
        System.out.println("Certificate pattern match: " + (CERTIFICATE_PATTERN.matcher(text).find()));
        ...
        try {
          ...
          Mat certificate = Imgcodecs.imread(file.getAbsolutePath(), Imgcodecs.IMREAD_GRAYSCALE);

          if ((logoMatched(preprocessImage(ursbLogo), preprocessImage(certificate)) || logoMatched(preprocessImage(coatOfArmsLogo), preprocessImage(certificate))) && (CERTIFICATE_PATTERN.matcher(text).find())){
